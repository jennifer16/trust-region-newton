 ...
 */
 ...
 #ifndef BoundaryPN_PROJECTED_NEWTON
  #define BoundaryPN_PROJECTED_NEWTON 1
#endif
...
// for vpn
#include "ElementDeformationState.h"

std::vector<double> g_element_J;
int g_reg_element_num = 0; // 用于记录进行 regularization的元素数量
double g_para_gamma = 2.0; // 初始化 g_para_gamma
...
TinyAD::HessianProjectionMode parse_projection_mode(const std::string& mode_str) {
    static const std::map<std::string, TinyAD::HessianProjectionMode> mode_map = {
        ...
        {"clamp_abs_blending_j", TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING_J},
        {"clamp_abs_blending_q", TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING_Q},
        {"clamp_abs_blending_j_energy", TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING_J_Energy},
...
// copy from vpn_reg_projected_newton
// test for 下界
int boundaryPN_reg_projected_newton(int argc, char** argv)
{
  TINYAD_DEBUG_OUT("#boundaryPN_reg_projected_newton" );
  // parse command line arguments
  cxxopts::Options options("Projected Newton with a trust region", "Choose eigenvalue filtering method: adaptive, clamp, abs");
  ...
  options.add_options()
   ...
   // ("beta", "Projection threshold for the eigenvalue projection", cxxopts::value<std::string>()->default_value("10.0"))// 光滑参数
    ...
    ("adaptive", "adaptive according to rho ", cxxopts::value<double>()->default_value("1e-5")) // >0 为adaptive
    ("enable_constraint", "enable constraints processing ", cxxopts::value<bool>()->default_value("false")) // 若设置diff，---
    ("para_gamma", "fixed gamma for blending_J ", cxxopts::value<double>()->default_value("2.0"))
  ...
  double para_gamma = result["para_gamma"].as<double>();
  g_para_gamma = para_gamma;
  ...
  */
  if (diff)
  {
      ...
      || _diff_mode == TinyAD::HessianProjectionMode::ABS_SHIFT_CLAMP_NONDIFF) {
        ...
        if (eps < 0)
        {
          eps = 0;
          eps = TinyAD::EPS_1E_8;
          TINYAD_WARNING("Warning: eps should be non-negative. Setting eps to a small positive value.");
        }
      }
  }
  else {
    ...
    + std::to_string(fixed_boundary_range)+"_diff_mode_" + diff_mode_str+ "/";
  {
    ...
    if (!std::filesystem::exists(output_folder + "energy_injection1/"))
      std::filesystem::create_directory(output_folder + "energy_injection1/");
    if (!std::filesystem::exists(output_folder + "energy_injection2/"))
      std::filesystem::create_directory(output_folder + "energy_injection2/");
    if (!std::filesystem::exists(output_folder + "obj_func/"))
      std::filesystem::create_directory(output_folder + "obj_func/");
  ...
  // print out the configuration
  {
    ...
    TINYAD_DEBUG_OUT("Adaptive method: " << adaptive);
    TINYAD_DEBUG_OUT("enable_constraint: " << enable_constraint);
    TINYAD_DEBUG_OUT("para_gamma: " << para_gamma);
    ...
    [&]()
    {
      ...
      */ 
      ...
      func.add_elements<4>(TinyAD::range(F.rows()), [&] (auto& element) -> TINYAD_SCALAR_TYPE(element) {
          ...
          auto Ic = (J.transpose() * J).trace();
          auto detF = J.determinant();
          g_element_J[f_idx] = TinyAD::to_passive(detF);  // ← 仅一行，零额外开销
          double alpha = 1.0 + mu / lambda;
          auto W = mu / 2.0 * (Ic - 3.0) + lambda / 2.0 * (detF - alpha) * (detF - alpha);
          return A * W;
      });
      ...
      double initial_energy = func.eval(x);
      TINYAD_DEBUG_OUT("Initial energy: " << initial_energy);
      ...
      else if (_diff_mode == TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING
      || _diff_mode == TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING_SMOOTH
      || _diff_mode == TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING2
      || _diff_mode == TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING3
      || _diff_mode == TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING5
      ...
      else if (_diff_mode == TinyAD::HessianProjectionMode::CLAMP_ABS_BLENDING_SHEAR)
      {
        hist_trust_region_ratio.push_back(1.0);
      }
      ...
      for (int i = 0; i < max_iters; ++i)
      {
        ...
        TINYAD_DEBUG_OUT("eps in iter " << i << ": " << eps);
        auto [f, g, H_proj] = func.eval_with_hessian_proj(x, eps, _diff_mode); // 
        Eigen::SparseMatrix<double> H0 = func.eval_hessian(x); // 计算未投影的Hessian，用于后续计算牛顿下降量和牛顿下降量
        ...
        TINYAD_DEBUG_OUT("d norm " << i << " =  " << d.norm());
        TINYAD_DEBUG_OUT("g norm " << i << " =  " << g.norm());
        TINYAD_DEBUG_OUT("H_proj norm " << i << " =  " << H_proj.norm());
      ...
      // output all the optimization statistics
      {
        ...
        std::ofstream output_file_gamma(output_folder + "obj_func/"  + "gamma.txt");
        std::ostream_iterator<double> output_gamma(output_file_gamma, "\n");
        std::copy(std::begin(hist_gamma), std::end(hist_gamma), output_gamma);

        std::ofstream output_file_reg_element_ratio(output_folder + "obj_func/"  + "reg_element_ratio.txt");
        ...
        std::copy(std::begin(hist_obj_func_energy), std::end(hist_obj_func_energy), output_obj_func_energy);

        std::ofstream output_file_obj_func_energy_ratio(output_folder + "obj_func/" + "f_energy_ratio.txt");
        std::ostream_iterator<double> output_obj_func_energy_ratio(output_file_obj_func_energy_ratio, "\n");
        std::copy(std::begin(hist_obj_func_energy_ratio), std::end(hist_obj_func_energy_ratio), output_obj_func_energy_ratio);
      ...
      CSVLineBuilder builder;
      builder.add(arr_time_str) //scalar value
            .add(arr_mesh_name)
            .add(arr_n_v)
            .add(arr_n_t)
            .add(arr_pos)
...
{
  TINYAD_DEBUG_OUT("#vpn_reg_projected_newton" );
  // parse command line arguments
  cxxopts::Options options("Projected Newton with a trust region", "Choose eigenvalue filtering method: adaptive, clamp, abs");
...
{
  #if BoundaryPN_PROJECTED_NEWTON
    //std::cout <<  << std::endl;
    TINYAD_DEBUG_OUT("Using boundaryPN_reg_projected_newton with trust region heuristic.");
    // vpn_reg_projected_newton(argc, argv);
    //shear_projected_newton(argc, argv);
    //shear_reg_projected_newton(argc, argv);// ok for negative eigenvalue
    //fs_projected_newton_global(argc, argv); // ok for smoothed face
    boundaryPN_reg_projected_newton(argc, argv);
  #elif SHEAR_PROJECTED_NEWTON
    //std::cout <<  << std::endl;
    TINYAD_DEBUG_OUT("Using vpn_reg_projected_newton with trust region heuristic.");
    //shear_projected_newton(argc, argv);
    //shear_reg_projected_newton(argc, argv);// ok for negative eigenvalue
    //fs_projected_newton_global(argc, argv); // ok for smoothed face
    vpn_reg_projected_newton(argc, argv);
