...
TEST(clamp01, OutOfRangeValues) {
    ...
    EXPECT_DOUBLE_EQ(clamp01(-100.0), 0.0);
...
TEST_F(HessianProjectionTest, ComputeKappa_Method1_PositiveAndNegative) {
    ...
    double kappa = computeKappa(1, eigenvalues, 1e-8);
    // λ_max = 3, λ_min = -2
...
TEST_F(HessianProjectionTest, ComputeKappa_Method2_AllPositive) {
    ...
    double kappa = computeKappa(2, eigenvalues, 1e-8);
...
TEST_F(HessianProjectionTest, ComputeKappa_Method3_AllPositive) {
    ...
    double kappa = computeKappa(3, eigenvalues, 1e-8);
...
TEST_F(HessianProjectionTest, ComputeKappa_Method4_MixedSign) {
    ...
    double kappa = computeKappa(4, eigenvalues, 1e-8);
    EXPECT_LE(kappa, 1.0);
    EXPECT_GE(kappa, 0.0);
}
...
TEST_F(HessianProjectionTest, ComputeAlphaJ_Method1_PositiveJ) {
    double J = 1.2;
    double alpha_J = computeAlpha_J(J, 1e-8);
...
TEST(ComputeAlphaHessianTest, AllPositiveEigenvalues) {
    ...
    double alpha = computeAlpha_hessian(eigenvalues, 1e-8);
...
TEST_F(OptimizeNegTest, Mode1_GradOnly) {
    g_neg_mode = 1;
    double result = optimizeVpnEigenvalue_neg(-5.0, 0.8, 0.5, 0.6, g_neg_mode, 1e-8);
...
TEST_F(OptimizeNegTest, Mode2_HessianOnly) {
    g_neg_mode = 2;
    double result = optimizeVpnEigenvalue_neg(-5.0, 0.8, 0.6, 0.6, g_neg_mode, 1e-8);
...
TEST_F(OptimizePosTest, Mode1_FixedWithEps) {
    g_pos_mode = 1;
    double result = optimizeVpnEigenvalue_pos(5.0, 0.8, 0.5, 0.6, g_pos_mode, 1e-8);
...
TEST_F(OptimizeEigenvalueTest, PositiveLambda) {
    double result = optimizeVpnEigenvalue(5.0, 0.8, 0.5, 0.6, 1e-8);
...
TEST_F(OptimizeEigenvalueTest, NearZeroLambda) {
    double result = optimizeVpnEigenvalue(1e-9, 0.8, 0.5, 0.6, 1e-8);
...
TEST(ComputeGradientProjectionTest, BasicFunctionality) {
    ...
    Eigen::VectorXd result = computeGradientProjection_vpn(eigenvectors, g);
    // 单位矩阵，投影等于原向量
...
TEST_F(HessianProjectionTest, ComputeAlphaGradEta_Mode2) {
    g_eta_mode = 2;
    double eta_pos = 0, eta_neg = 0;
    computeAlpha_grad_eta(eta_pos, eta_neg, 0.8);
...
TEST_F(HessianProjectionTest, ComputeAlphaGradKappa_Mode1) {
    ...
    Eigen::VectorXd eigenvalues(3);
    eigenvalues << -2.0, 1.0, 3.0;
    double kappa = computeAlpha_grad_kappa(eigenvalues, 1e-8, 1.0);
    EXPECT_LE(kappa, 1.0);
    EXPECT_GE(kappa, 0.0);
}
...
TEST_F(HessianProjectionTest, ComputeAlphaGradKappa_Mode2) {
    ...
    Eigen::VectorXd eigenvalues(3);
    eigenvalues << -2.0, 1.0, 3.0;
    double kappa = computeAlpha_grad_kappa(eigenvalues, 1e-8, 0.6);
...
TEST_F(HessianProjectionTest, ComputeAlphaGradKappa_Mode3) {
    ...
    Eigen::VectorXd eigenvalues(3);
    eigenvalues << -2.0, 1.0, 3.0;
    double kappa = computeAlpha_grad_kappa(eigenvalues, 1e-8, 0.3);
